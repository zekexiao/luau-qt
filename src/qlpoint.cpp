...
int qlpoint_gc(lua_State *L) {
    auto udata = (QPointF **) luaL_checkudata(L, 1, QLPOINT_METATABLE_NAME);
    delete *udata;
    return 0;
}
...
void qlpoint_openlib(lua_State *L) {
    luaL_newmetatable(L, QLPOINT_METATABLE_NAME);

    lua_pushcfunction(L, qlpoint_gc, "__gc");
    lua_setfield(L, -2, "__gc");

    lua_pushcfunction(L, qlpoint_index, "__index");
    lua_setfield(L, -2, "__index");

    lua_pushcfunction(L, qlpoint_newindex, "__newindex");
    lua_setfield(L, -2, "__newindex");

    lua_pop(L, 1);

    lua_newtable(L);
    lua_pushcfunction(L, qlpoint_create, "create");
